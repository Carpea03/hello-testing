...
def generate_output(input_text, patent_details, example_output_urls):
    ...
    response = client.messages.create(
        ...
        system=f"Act as an expert Australian patents attorney working for Baxter IP Pty Ltd. Here are some example outputs to guide your response:\n\n{''.join(example_outputs)}. Remember that the 'final date for acceptance' is the date to which all issues that are raised by IP Australia must be resolved otherwise the application lapses.",
        messages=[
            {
                "role": "user",
                "content": [
